...
[ -d "$HOME/Projects/ai-setup" ] || {
  echo ">> Cloning ai-setup into ~/Projects/ai-setup"
  git clone --depth 1 "$REPO_URL" "$HOME/Projects/ai-setup"
}
...
Bootstrap done. Manual steps (interactive / secret, cannot be automated):
  ...
  3. opencode run "$(cat ~/Projects/ai-setup/opencode-setup-prompt.md)"
