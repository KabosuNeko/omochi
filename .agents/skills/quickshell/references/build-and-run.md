...
```sh
qs --path ./config/quickshell --no-duplicate
qs --path ./shell.qml --no-duplicate
```

Run a named config installed under an XDG config root:

```sh
qs --config <name> --no-duplicate
```
...
- Check `qs list`, `qs log`, and `qs ipc` when a project defines IPC.
